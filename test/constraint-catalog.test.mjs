...
import { discoverConstraints } from '../scripts/discover-constraints.mjs';
import { renderConstraintCatalog } from '../scripts/render-constraints.mjs';
import { compileConstraintRules } from '../scripts/compile-constraint-rules.mjs';
import { constraintRoles } from '../scripts/constraint-rule-view.mjs';
import { collectRuleHistory } from '../scripts/constraint-rule-history.mjs';
...
test('discovers reference closure, records exclusions, ignores fenced examples and separates review', () => {
  ...
  try {
    ...
    const payload = JSON.parse(html.match(/id="birdview-constraint-data">([\s\S]*?)<\/script>/)[1]);
    ...
    assert.ok(combined.includes("nav.id = 'project-views'"));
    ...
    const rulePayload = JSON.parse(ruleHtml.match(/id="birdview-constraint-data">([\s\S]*?)<\/script>/)[1]);
    ...
    reviewed.rules[0].targetRole = 'testing';
    ...
    assert.equal(versioned.rules[0].history.version, 1);
    ...
    versioned.rules[0].history.snapshot = '0'.repeat(40);
    ...
    catalog.rules = [{ id: 'reviewed', sourcePath: 'AGENTS.md', line: 500, endLine: 501 }];
    ...
    assert.equal(collectRuleHistory(reviewed, clone).rules[0].history.reason, 'shallow-history');
