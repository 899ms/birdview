...
export function compileConstraintRules(catalog, selection) {
  ...
  for (const group of selection.groups) {
    ...
    for (const authored of group.rules) {
      ...
      const { anchor, history, ...rule } = authored;
      rules.push({ ...rule, sourcePath: source.path, line: start + 1, endLine: end + 1,
        category: group.category, topic: group.topic, applicability: rule.applicability || 'conditional' });
    }
  }
  return { ...catalog, architectureBinding: selection.architectureBinding, rules, ruleReview: { scope: selection.scope, sourcePaths: [...new Set(rules.map(rule => rule.sourcePath))],
