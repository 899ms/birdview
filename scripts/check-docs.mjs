...
if (!update) {
  const saved = fs.existsSync(record) ? JSON.parse(fs.readFileSync(record, 'utf8')) : {};
  for (const file of new Set([...Object.keys(saved), ...Object.keys(hashes)])) {
    if (saved[file] !== hashes[file]) errors.push(`${file}: synchronization confirmation required`);
  }
}
if (errors.length) {
