...
export function validate(map, events = [], { requireBilingual = false, requireRoles = false } = {}) {
  ...
  for (const [index, rule] of (map.constraints || []).entries()) {
    ...
    for (const source of rule.evidence) if (source.endLine !== undefined && source.endLine < source.line) error('evidence/line-order', location, 'endLine precedes line.');
  }
  for (const rule of constraints.values()) {
    const references = [rule.supersededBy, ...(rule.conflictsWith || [])].filter(Boolean);
