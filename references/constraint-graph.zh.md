...
检查两个视图、切换保留状态、主题变化、中英文导航、窄屏、模块筛选和来源索引链接。保留原架构布局。即使配套约束来自真实源码历史，概念演示架构仍须注明是概念图。
