...
前提是用户授权任务和[阶段 1](map-project.zh.md) 建立的可用地图。
...
每行 JSONL 都携带完整范围和目标。编辑新增模块前，用新的 `planned` 解释范围扩大。有归属文件的编辑应将所有匹配所有者列为目标；无归属文件同时列入 `files` 与 `unmappedFiles`。不能为通过校验编造归属，误导性规则应回到阶段 1 修正。
